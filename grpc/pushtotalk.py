...
import pygame
import threading
from .Modules.core import Worker
###-------------------------------------###
...
class SampleAssistant(object):
    ...
    def converse(self):
        ...
        for resp in self.assistant.Converse(iter_converse_requests(),
            ...
            ###-------------------------------------------------------###
            if custom == None:
                custom = customCall(self.worker, resp.result.spoken_request_text)
                if custom == True:
                    logging.info("Custom: %s", continue_conversation)
                if custom == False:
                    logging.info("Not Custom: %s", continue_conversation)
            ...
            ###-------------------------------------------------------###
            if len(resp.audio_out.audio_data) > 0 and custom == True:
                # logging.info("Custom: %s", resp.audio_out.audio_data)
            ###-------------------------------------------------------###
...
def customCall(worker, speech):
    if "Google" in speech:
        return True
    elif "무드" in speech:
        if "불 켜" in speech:
            if worker != None:
                worker.work({"state": "on"})
            answer = "네 불을 켭니다"
        elif "불 꺼" in speech:
            if worker != None:
                worker.work({"state": "off"})
            answer = "네 불을 끕니다"
        else:
            answer = "네 무드등입니다"
        thr = threading.Thread(target=speak, args=(answer, ))        
        thr.start()
        return False
    else:
        return False
...
def speak(text):
    filePath = '/home/pi/voice.mp3'
    if os.path.isfile(filePath):
        os.remove(filePath)
    eng_wav = gTTS(text, lang = 'ko') 
    eng_wav.save(filePath)
    pygame.mixer.init()
